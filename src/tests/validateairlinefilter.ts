...
const airlinePayload = require('../resources/airlines.data.json');

export class ValidateAirlineFilter {
    

    validateAirlineFilter = async (flightCode:string) =>  {
        ...
        let config = {
            ...
            data: data.replace('<FLIGHTS_CODE>', flightCode)
        };
        ...
        // }
        const flightName:string = resp.data.data.search.flights[0].bounds[0].segments[0].operatingCarrier.name;
        const flightCodeShort:string = resp.data.data.search.flights[0].bounds[0].segments[0].operatingCarrier.code;
        return { flightName, flightCodeShort };    
    }
}

for ( let airline of airlinePayload ){
    console.log(`Request -  Airline: ${airline.value} | ${airline.code}`);
   new ValidateAirlineFilter().validateAirlineFilter(airline.value).then( (flName)=>{
        console.log(`Response Flights: `, flName.flightCodeShort, flName.flightName);
    });
