...
const numberOfStopsPayload = require('../resources/numberofstops.data.json');
...
export class ValidateAirlineFilter {
    ...
    validateAirlineFilter = async (flightCode:string) =>  {
        ...
        let config = {
            ...
            data: data.replace('<STOP_COUNT>', flightCode)
        };
        ...
        // }
        const numberOfStops:string = resp.data.data.search.flights[0].bounds[0].segments[0].numberOfTechnicalStops;
   
        return { numberOfStops };    
    }
}

for ( let stops of numberOfStopsPayload ){
    console.log(`Request -  Number of Stops: ${stops.value} | ${stops.code}`);
   new ValidateAirlineFilter().validateAirlineFilter(stops.value).then( (flName)=>{
        console.log(`Response Flights: `, flName.numberOfStops);
    });
